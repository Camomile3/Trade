...
void SYSTEM::BuySys(int ID) {
	...
	DrawStringToHandle(MWX + 64 + 32, MWY + 64 , "Ç¢Ç≠Ç¬çwì¸ÇµÇ‹Ç∑Ç©ÅH", GetColor(255, 255, 255), init.FontHandle);
	...
	if (TempPrice > her.Money) {
		ResetCity();
		DrawStringToHandle(MWX + 64 + 32, MWY + 64 , "èäéùã‡Ç™ë´ÇËÇ‹ÇπÇÒ", GetColor(255, 255, 255), init.FontHandle);
		WaitClick();
	}
	else if (TempNumber + her.CargoWeight > her.MaxWeight) {
		ResetCity();
		DrawStringToHandle(MWX + 64 + 32, MWY + 64 , "èdó ÉIÅ[ÉoÅ[Ç≈Ç∑", GetColor(255, 255, 255), init.FontHandle);
		WaitClick();
	}
	else if (TempNumber != 0)
	{
		...
		if (her.Cargo[SlotNumber] != 0) {
			ResetCity();
			DrawStringToHandle(MWX + 64 + 32, MWY + 64 , "çwì¸ÇµÇ‹ÇµÇΩ", GetColor(255, 255, 255), init.FontHandle);
			WaitClick();
		}
	}
	BtnSwitch = Sw_BUY;
...
void SYSTEM::SaleSys(int ID) {
	...
	DrawStringToHandle(MWX + 64 + 32, MWY + 64 , "Ç¢Ç≠Ç¬îÑãpÇµÇ‹Ç∑Ç©ÅH", GetColor(255, 255, 255), init.FontHandle);
	printfDx("SlotNumber=%d\n", SlotNumber);
	...
	if (TempNumber > her.Cargo[SlotNumber]) {
		ResetCity();
		DrawStringToHandle(MWX + 64 + 32, MWY + 64 , "èäóLó Çí¥Ç¶ÇƒÇ¢Ç‹Ç∑", GetColor(255, 255, 255), init.FontHandle);
		while (1) {
	...
	else if (TempNumber != 0)
	{
		...
		ResetCity();
		DrawStringToHandle(MWX + 64 + 32, MWY + 64 , "îÑãpÇµÇ‹ÇµÇΩ", GetColor(255, 255, 255), init.FontHandle);
		while (1) {
			if (CheckHitKey(KEY_INPUT_NUMPADENTER) == 0) {
				if (CheckHitKey(KEY_INPUT_RETURN) == 0) {
					while (1) {
...
void SYSTEM::InvestBuySys(int ID) {
	...
	if (TempPrice > her.Money) {
		ResetCity();
		DrawStringToHandle(MWX + 64 + 32, MWY + 64 , "èäéùã‡Ç™ë´ÇËÇ‹ÇπÇÒÅB", GetColor(255, 255, 255), init.FontHandle);
		WaitClick();
	}
	else if (trans[ID].Sea == FALSE && TempNumber + her.MaxWeight > 999999999) {
		ResetCity();
		DrawStringToHandle(MWX + 64 + 32, MWY + 64 , "Ç±ÇÍà»è„îÉÇ¶Ç‹ÇπÇÒÅB", GetColor(255, 255, 255), init.FontHandle);
		WaitClick();
	}
	...
	else if (TempNumber != 0)
	{
		...
		her.Money -= TempPrice;
		ResetCity();
		DrawStringToHandle(MWX + 64 + 32, MWY + 64 , "çwì¸ÇµÇ‹ÇµÇΩÅB", GetColor(255, 255, 255), init.FontHandle);
		WaitClick();
	}
	BtnSwitch = Sw_INVEST;
	ResetCity();
...
void SYSTEM::InvestSaleSys(int ID) {
	...
	if (her.Transport[ID] < TempNumber) {
		ResetCity();
		DrawStringToHandle(MWX + 64 + 32, MWY + 64 , "èäéùêîÇí¥Ç¶ÇƒÇ¢Ç‹Ç∑ÅB", GetColor(255, 255, 255), init.FontHandle);
		WaitClick();
	}
	else if (TempNumber != 0)
	{
		...
		her.Money += TempPrice;
		ResetCity();
		DrawStringToHandle(MWX + 64 + 32, MWY + 64 , "îÑãpÇµÇ‹ÇµÇΩÅB", GetColor(255, 255, 255), init.FontHandle);
		WaitClick();
	}
	BtnSwitch = Sw_INVEST;
	ResetCity();
