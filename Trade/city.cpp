...
void SYSTEM::InvestBuySys(int ID) {
	...
	if (TempPrice > her.Money) {
		ResetCity();
		DrawStringToHandle(MWX + 64, MWY + 64, "èäéùã‡Ç™ë´ÇËÇ‹ÇπÇÒÅB", GetColor(255, 255, 255), init.FontHandle);
		WaitClick();
	}
	else if (trans[ID].Sea == FALSE && TempNumber + her.MaxWeight > 999999999) {
		ResetCity();
		DrawStringToHandle(MWX + 64, MWY + 64, "Ç±ÇÍà»è„îÉÇ¶Ç‹ÇπÇÒÅB", GetColor(255, 255, 255), init.FontHandle);
		WaitClick();
	}
	...
	else if (TempNumber != 0)
	{
		...
		her.Money -= TempPrice;
		ResetCity();
		DrawStringToHandle(MWX + 64, MWY + 64, "çwì¸ÇµÇ‹ÇµÇΩÅB", GetColor(255, 255, 255), init.FontHandle);
		WaitClick();
	}
	BtnSwitch = Sw_INVEST;
	ResetCity();
...
void SYSTEM::InvestSaleSys(int ID) {
	...
	if (her.Transport[ID] < TempNumber) {
		ResetCity();
		DrawStringToHandle(MWX + 64, MWY + 64, "èäéùêîÇí¥Ç¶ÇƒÇ¢Ç‹Ç∑ÅB", GetColor(255, 255, 255), init.FontHandle);
		WaitClick();
	}
	else if (TempNumber != 0)
	{
		...
		her.Money += TempPrice;
		ResetCity();
		DrawStringToHandle(MWX + 64, MWY + 64, "îÑãpÇµÇ‹ÇµÇΩÅB", GetColor(255, 255, 255), init.FontHandle);
		WaitClick();
	}
	BtnSwitch = Sw_INVEST;
	ResetCity();
