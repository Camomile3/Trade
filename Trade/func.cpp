...
void BUTTON::QuitWindow() {
	...
	DrawStringToHandle(mw.MWX + 64, mw.MWY + 64, "I—¹‚µ‚Ü‚·‚©H", GetColor(255, 255, 255), FontHandle);
	DrawStringToHandle(BtnX[4], BtnY[4], "‚Í‚¢", GetColor(255, 255, 255), FontHandle);
	DrawStringToHandle(BtnX[5], BtnY[5], "‚¢‚¢‚¦", GetColor(255, 255, 255), FontHandle);
		...
		while (MOver == FALSE) {
			...
			for (int i = 4; i < 6; i++) {
				if ((MouseX >= BtnX[i] && MouseX <= BtnX[i] + BtnW[i]) && (MouseY >= BtnY[i] && MouseY <= BtnY[i] + BtnH[i])) {
					switch (i) {
					case 4:
						DrawStringToHandle(BtnX[4], BtnY[4], "‚Í‚¢", GetColor(255, 0, 0), FontHandle);
						break;
					case 5:
						DrawStringToHandle(BtnX[5], BtnY[5], "‚¢‚¢‚¦", GetColor(255, 0, 0), FontHandle);
						break;
					}
					MOver = TRUE;
					OveredBtn = i;
